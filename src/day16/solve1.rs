...
fn follow_beam(beam: BeamState, contraption: &Vec<Vec<char>>, beam_history: Vec<BeamState>) -> Vec<BeamState> {
    ...
    if out_of_bounds || loop_detected { return beam_history.clone() }
...
pub fn solve() {
    ...
    let beam = BeamState { direction: Direction::Right, pos: (0, 0) };

    let mut energized_tiles = follow_beam(beam, &contraption, Vec::new()).to_vec().iter().map(|b| b.pos).collect::<Vec<(isize, isize)>>();
    energized_tiles.sort();
    energized_tiles.dedup();

    println!("Sum: {}", energized_tiles.len());
}
